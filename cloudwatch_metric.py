...
class EC2WithCloudWatchAgentStack(core.Stack):
    def __init__(self, scope: core.Construct, id: str, **kwargs) -> None:
        ...
        # Install the CloudWatch agent using user data
        instance.user_data.add_commands(
            "sudo yum -y install amazon-cloudwatch-agent",
            "sudo tee /etc/amazon/amazon-cloudwatch-agent.json <<EOF\n" +
            self.generate_agent_config() +
            "\nEOF\n",
            "sudo /opt/aws/amazon-cloudwatch-agent/bin/amazon-cloudwatch-agent-ctl -a start -m ec2 -c file:/etc/amazon/amazon-cloudwatch-agent.json",
        )
        ...
        ssm.StringParameter(self, "CloudWatchAgentConfigParameter",
            ...
            string_value=self.generate_agent_config()
        )
